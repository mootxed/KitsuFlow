# 6ba820ae-4429-44c6-97fd-40986be6ff82 implementation handoff

This archive is the source of truth for turning the design into production code. Start from `kitsuflow-workspace.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
- Primary entry: `kitsuflow-workspace.html`
...
- A single self-contained `kitsuflow-workspace.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `kitsuflow-workspace.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `kitsuflow-workspace.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
